...
Page({
    ...
    _toPayTap: function (orderId, money) {
        ...
        if (money <= 0) {
            ...
            wechatPay.wechatPay('order', money, orderId, "/pages/order-list/index");
        }
    },
    ...
    getOrderStatistics() {
        WXAPI.orderStatistics().then(res => {
            if (res.code === 0) {
                ...
                badges[1] = res.data.noPay
                badges[2] = res.data.noShipped
                badges[3] = res.data.noConfirm
                badges[4] = res.data.noReview
                this.setData({
                    badges
                })
            }
    ...
    doneShow() {
        ...
        if (this.data.hasRefund) {
            fetchData.hasRefund = true
        }
        if (!fetchData.hasRefund) {
            fetchData.status = that.data.status;
        }

        this.getOrderStatistics();
        WXAPI.orderList(fetchData).then(function (res) {
            if (res.code === 0) {
                that.setData({
                    orderList: res.data.rows,
                    logisticsMap: res.data.logisticsMap,
                    orderProduct: res.data.rows.orderProduct,
                });
            } else {
                that.setData({
                    orderList: null,
